...
class SWEBenchWrapper(SimpleResponsesAPIAgent):
    ...
    async def responses(
        ...
        model_endpoint = get_model_endpoint(
            self.config.model_server.name if self.config.model_server else None
        )
    ...
    async def run(self, body: SWEBenchRunRequest) -> SWEBenchVerifyResponse:
        ...
        update_dict['parallel_tool_calls'] = (
            False if self.config.agent_framework == "swe_agent" else True
        )
        ...
        params_with_input = fixed_params.model_copy(update={
            'input': input_messages,
            'tools': response.tools if response.tools else []
        })
        ...
        patch_applied = metrics.get("patch_successfully_applied") or (metadata.get("patch_successfully_applied") == "True")
