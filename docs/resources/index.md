# Resources

Quick reference materials, glossaries, and helpful guides to support your work with NeMo Gym.

---

## Available Resources

::::{grid} 1 1 2 2
:gutter: 3

:::{grid-item-card} 📖 Glossary
:link: glossary
:link-type: doc

Essential terminology for agent training, RL workflows, and NeMo Gym architecture.
:::

::::

---

## What Are Resources?

Resources are reference materials designed for quick lookup rather than sequential reading. Unlike tutorials (learning-oriented) or concepts (understanding-oriented), resources provide:

- **Quick definitions** - Find terminology fast
- **Reference tables** - Compare options and patterns
- **Checklists** - Verify configurations and setups
- **Examples** - Common patterns and code snippets

---

## Using This Section

**When to use resources**:
- ✓ Looking up unfamiliar terms
- ✓ Need a quick reminder of concepts
- ✓ Comparing configuration options
- ✓ Finding code examples

**When to use other sections**:
- Tutorials → Learn by doing (hands-on)
- Concepts → Understand why and how (explanation)
- How-to guides → Solve specific problems (task-oriented)

